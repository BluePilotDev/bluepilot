...
class Controls:
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(CC.longActive, CS, long_plan.aTarget, long_plan.shouldStop, pid_accel_limits, resume, self.sm['longitudinalPlanSP'].mpcSource)
    ...
    if lac_log.active and not recent_steer_pressed and not self.CP.notCar and CS.madsEnabled:
      if self.sm['carOutput'].actuatorsOutput.fordVariables.steeringLimited01 > 0:
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    if current_alert:
      ...
      ### Get DM state to pass to HUDControl for vehicles with specific cluster messages for this. ###
      if current_alert.alert_type == "preDriverDistracted/permanent":
        hudControl.driverState = "preDistracted"
      elif current_alert.alert_type == "promptDriverDistracted/permanent":
        hudControl.driverState = "promptDistracted"
      elif current_alert.alert_type == "driverDistracted/permanent":
        hudControl.driverState = "distracted"
      elif current_alert.alert_type == "preDriverUnresponsive/permanent":
        hudControl.driverState = "preUnresponsive"
      elif current_alert.alert_type == "promptDriverUnresponsive/permanent":
        hudControl.driverState = "promptUnresponsive"
      elif current_alert.alert_type == "driverUnresponsive/permanent":
        hudControl.driverState = "unresponsive"
      else:
        hudControl.driverState = "none"

      ### Get disable state to pass to HUDControl for vehicles with specific cluster messages for this. ###
      if current_alert.event_type == ET.USER_DISABLE:
        hudControl.disableState = "userDisable"
      elif current_alert.event_type == ET.SOFT_DISABLE:
        hudControl.disableState = "softDisable"
      elif current_alert.event_type == ET.IMMEDIATE_DISABLE:
        hudControl.disableState = "immediateDisable"
      elif current_alert.event_type == ET.NO_ENTRY:
        hudControl.disableState = "noEntry"
      else:
        hudControl.disableState = "none"
