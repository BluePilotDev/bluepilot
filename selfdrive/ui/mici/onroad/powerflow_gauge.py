...
class MiciPowerflowGauge(Widget):
  ...
  def draw_arc_segment(self, angle, color):
    ...
    rl.draw_line_ex(rl.Vector2(x1, y1), rl.Vector2(x2, y2), 6, color)
  ...
  def draw_vertical_gauge(self, rect: rl.Rectangle, value):
    ...
    rl.draw_rectangle(bar_x, bar_y, BAR_W, bar_h, rl.Color(0, 0, 0, 100))

    demand_h = mid * abs(value)
    regen_h = (bar_h - mid) * abs(value)
    if value > 0:
      rl.draw_rectangle(bar_x, int(mid - demand_h), BAR_W, int(demand_h), POWERFLOW_DEMAND_COLOR)
    elif value < 0:
      rl.draw_rectangle(bar_x, int(mid), BAR_W, int(regen_h), POWERFLOW_REGEN_COLOR)
    ...
    for i in range(LINES):
      y = int(segment_h * i)
      rl.draw_line(bar_x, y, bar_x + 5, y, line_color)
      rl.draw_line(bar_x, int(y - 1), bar_x + 5, int(y - 1), line_shadow)
      rl.draw_line(bar_x + BAR_W - 5, y, bar_x + BAR_W, y, line_color)
      rl.draw_line(bar_x + BAR_W - 5, int(y - 1), bar_x + BAR_W, int(y - 1), line_shadow)

    rl.draw_rectangle(bar_x, int(mid), BAR_W, 2, rl.WHITE)
    rl.draw_line(bar_x, int(mid + 2), bar_x + BAR_W, int(mid + 2), line_shadow)
