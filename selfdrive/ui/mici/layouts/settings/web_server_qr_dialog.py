...
class WebServerQRDialog(NavWidget):
  ...
  def _get_wifi_ip(self) -> str:
    ...
    try:
      for iface in ['wlan1', 'wlan2']:
        result = subprocess.run(['ip', 'addr', 'show', iface], capture_output=True, text=True, timeout=2)
        for line in (result.stdout or '').split('\n'):
          ip = _parse_ip_from_line(line)
          if ip:
            return ip
  ...
  def _handle_toggle(self, checked: bool):
    if not checked and self._back_callback:
      self._back_callback()
  ...
  def _render(self, _):
    ...
    if url:
      ...
      self._scan_label.set_position(qr_x, qr_y + qr_size + 56)
    ...
    toggle_rect = rl.Rectangle(right_x, title_y + 80, right_width, 60)
  ...
  def _render_qr_code(self, rect: rl.Rectangle, has_url: bool = True) -> None:
    ...
    rl.draw_texture_ex(self._qr_texture, rl.Vector2(rect.x, rect.y), 0.0, scale, rl.WHITE)

  def _handle_mouse_release(self, mouse_pos):
    toggle_rect = self._disable_toggle._rect
    if toggle_rect and rl.check_collision_point_rec(mouse_pos, toggle_rect):
