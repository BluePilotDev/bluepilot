...
class BluePilotLayoutMici(NavWidget):
  def __init__(self, back_callback: Callable):
    ...
    self.custom_path_offset = BigParamFloatControl("in-lane offset", "custom_path_offset", en_param="enable_lane_positioning", min=-0.5, max=0.5, tint=rl.GREEN)
    self.enable_lane_full_mode = BigParamControl("enable lanefull mode", "enable_lane_full_mode", en_param="enable_lane_positioning", tint=rl.GREEN)
    self.custom_profile = BigParamControl("use custom tuning profile", "custom_profile", tint=rl.BLUE)
    self.pc_blend_ratio_high_C = BigParamFloatControl("predicted curvature blend ratio high", "pc_blend_ratio_high_C_UI", en_param="custom_profile", min=0.0, max=1.0, tint=rl.BLUE)
    self.pc_blend_ratio_low_C = BigParamFloatControl("predicted curvature blend ratio low", "pc_blend_ratio_low_C_UI", en_param="custom_profile", min=0.0, max=1.0, tint=rl.BLUE)
    self.LC_PID_gain = BigParamFloatControl("low curvature PID gain", "LC_PID_gain_UI", en_param="custom_profile", min=0.0, max=5.0, tint=rl.BLUE)
