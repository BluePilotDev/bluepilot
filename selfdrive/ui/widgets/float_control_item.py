...
class FloatControlAction(ItemAction):
  ...
  def _render(self, rect: rl.Rectangle) -> bool:
    ...
    minus_rect = rl.Rectangle(rect.x, button_y, BUTTON_SIZE, BUTTON_SIZE)
    self._minus_button.set_enabled(current_value > self.min_value and is_enabled)
    self._minus_button.render(minus_rect)
    
    # Plus button on right
    plus_rect = rl.Rectangle(rect.x + rect.width - BUTTON_SIZE, button_y, BUTTON_SIZE, BUTTON_SIZE)
    ...
    value_rect = rl.Rectangle(
      rect.x + BUTTON_SIZE + BUTTON_SPACING,
      rect.y,
      rect.width - (BUTTON_SIZE + BUTTON_SPACING) * 2,
      rect.height
    )
    gui_label(
      value_rect,
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    ...
    minus_rect = rl.Rectangle(self._rect.x, button_y, BUTTON_SIZE, BUTTON_SIZE)
    if rl.check_collision_point_rec(mouse_pos, minus_rect):
      self._minus_button._handle_mouse_release(mouse_pos)
      return
    
    plus_rect = rl.Rectangle(self._rect.x + self._rect.width - BUTTON_SIZE, button_y, BUTTON_SIZE, BUTTON_SIZE)
