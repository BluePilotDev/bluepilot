...
from openpilot.selfdrive.car.ford.helpers import (
    initialize_param_defaults, update_settings_params, load_initial_cc_pref_params,
    get_ford_vehicle_tuning_carcontroller, actuators_calc, logDebug
)
...
DriverState = car.CarControl.HUDControl.DriverState  # DM State
DisableState = car.CarControl.HUDControl.DisableState  # Disable State
  ...
  def update(self, CC, CS, now_nanos, model_data=None):
    ...
    if self.send_driver_monitor_can_msg:
      if self.send_hands_free_cluster_msg:
        if hud_control.disableState == DisableState.noEntry:
          tja_msg = 4 # BlueCruise not available
        elif (hud_control.driverState in (DriverState.distracted, DriverState.unresponsive) or hud_control.disableState in (DisableState.softDisable, DisableState.immediateDisable)):
          tja_warn = 3 # Resume Control
        elif hud_control.disableState == DisableState.userDisable:
          tja_warn = 1 # Cancelled
        elif hud_control.driverState == DriverState.preDistracted:
          tja_warn = 6 # Watch The Road (no chime)
        elif hud_control.driverState == DriverState.promptDistracted:
    ...
    if (self.frame % CarControllerParams.STEER_STEP) == 0:
      if CC.latActive:
        ...
        if model_data is not None and len(model_data.orientation.x) >= CONTROL_N:
          ...
          pc_blend_ratio = interp(desired_curvature, self.pc_blend_ratio_bp, self.pc_blend_ratio_v)
