...
def get_steer_angle_rate_limit(apply_angle: float, apply_angle_last: float, v_ego: float, limits: AngleSteeringLimits) -> float:
  # pick angle rate limits based on wind up/down
  steer_up = apply_angle_last * apply_angle >= 0. and abs(apply_angle) > abs(apply_angle_last)
  rate_limits = limits.ANGLE_RATE_LIMIT_UP if steer_up else limits.ANGLE_RATE_LIMIT_DOWN

  return np.interp(v_ego, rate_limits[0], rate_limits[1])
...
def apply_std_steer_angle_limits(apply_angle: float, apply_angle_last: float, v_ego: float, steering_angle: float,
  ...
  if lat_active:
    angle_rate_lim = get_steer_angle_rate_limit(apply_angle, apply_angle_last, v_ego, limits)
