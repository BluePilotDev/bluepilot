...
class LateralAngleExt:
  ...
  def _reset_angle_signals(self, CS):
    """Clear wire/telemetry state and the calibration evidence staging. Shared by the
  ...
  def update_angle_strategy(self, CC, CS, actuators, CP):
    ...
    _kappa_entering = _kappa_at_entering > abs(desired_curvature)
    if _kappa_entering:
    ...
    # As the curve grows the signal needs a boost to not understeer.
