...
# against angle_meas). Whenever the planner kappa cannot honestly describe the car's
# steering -- inactive, human-turn override, stall blip, driver pressing -- the published
# shadow must equal the measured curvature, so the panda-latched value always stays inside
# the check's band and re-engage frames never compare a stale zero against real measured
# curvature.
...
from opendbc.car import structs, scale_tire_stiffness
...
def _harness():
  CP = _explorer_cp()
  ext = _Harness(CP, structs.CarParamsSP())
